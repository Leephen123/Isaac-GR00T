...
class ClientConfig:
    ...
    history_len: int = 50
    roll_out: int = 20000
    action_exec_s: int = 35
    init_delay_frames: int = 6
    delay_buffer_len: int = 3
...
class DelayBuffer:
    def __init__(self, initial_delay_frames: int, maxlen: int):
        self._lock = threading.Lock()
        self._queue = deque(
            [max(int(initial_delay_frames), 0)],
...
def combine_action_chunks(
    new_actions: np.ndarray,
    previous_actions: np.ndarray | None,
    executed_steps: int,
    estimated_delay_steps: int,
) -> np.ndarray:
    """Fuse a new model output with the unexecuted suffix of the previous chunk."""
    new_actions = np.asarray(new_actions, dtype=np.float32).reshape(-1, 102).copy()
    if previous_actions is None:
        return new_actions

    previous_actions = np.asarray(previous_actions, dtype=np.float32).reshape(-1, 102)
    remaining_actions = previous_actions[executed_steps:]
    fusion_length = min(new_actions.shape[0], remaining_actions.shape[0])
    if fusion_length == 0:
        return new_actions

    frozen_steps = min(max(int(estimated_delay_steps), 0), fusion_length)
    weights = np.zeros(fusion_length, dtype=np.float32)
    weights[:frozen_steps] = 1.0

    decay_steps = fusion_length - frozen_steps
    if decay_steps > 1:
        rho = np.linspace(0.0, 1.0, decay_steps, dtype=np.float32)
        g = rho * (np.exp(rho) - 1.0) / (np.e - 1.0)
        weights[frozen_steps:fusion_length] = 1.0 - g
    elif decay_steps == 1:
        weights[frozen_steps] = 0.0

    weights = np.clip(weights, 0.0, 1.0)
    new_actions[:fusion_length] = (
        weights[:, None] * remaining_actions[:fusion_length]
        + (1.0 - weights[:, None]) * new_actions[:fusion_length]
    )
    return new_actions
...
class G1Gr00tAsyncRunner:
    def __init__(self, config: ClientConfig):
        ...
        self.delay_buffer = DelayBuffer(
            initial_delay_frames=config.init_delay_frames,
            maxlen=config.delay_buffer_len,
        )
    ...
    def get_actions(self) -> tuple[np.ndarray, np.ndarray, np.ndarray, int, int]:
        ...
        executed_steps = (
            0
            if previous_inference_start_step is None
            else inference_start_step - previous_inference_start_step
        )
        ...
        action_chunk_rel = np.asarray(response[0]["mocap"][0], dtype=np.float32)

        action_chunk_rel = combine_action_chunks(
            new_actions=action_chunk_rel,
            previous_actions=previous_action_chunk_rel,
            ...
            if should_update:
                ...
                with self.actions_lock:
                    if self.previous_inference_start_step is not None:
                        self.delay_buffer.add(inference_delay_steps)
