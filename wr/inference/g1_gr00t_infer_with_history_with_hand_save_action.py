...
class ClientConfig:
    ...
    # When enabled with --save-action, record every action frame sent to the robot.
    save_action: bool = False
    action_output_path: Path = Path("/output/action_pred.json")
    roll_out: int = 20000
        ...
        serialized_action = json.dumps(
            {
                ...
                "hand_cmd": np.asarray(hand_cmd, dtype=np.float32).tolist(),
            },
            allow_nan=False,
            separators=(",", ":"),
        )
...
if __name__ == "__main__":
    # Tyro exposes dataclass fields with hyphens, while this alias preserves the
    # requested --save_action spelling for existing launch commands.
    sys.argv = ["--save-action" if arg == "--save_action" else arg for arg in sys.argv]
