...
class ClientConfig:
    ...
    # The only motion-shaping knob. 0 disables shaping; 0.5 is the balanced
    # profile calibrated from G:\project\data.json; 1 is strongest smoothing.
    # It affects mocap positions only. Quaternion commands stay raw to avoid
    # wrist-orientation lag and rotation overshoot.
...
def limits_from_smoothing_strength(strength: float) -> MotionLimits | None:
    """Map one intuitive strength value to a tested position-limit profile."""
    if not 0.0 <= strength <= 1.0:
        raise ValueError(f"motion_smoothing must be in [0, 1], got {strength}")
    if strength == 0.0:
        return None

    # Each row is: smoothing strength, max speed (m/s), acceleration (m/s^2),
    # jerk (m/s^3). The 0.5 row is based on the real 50 Hz demonstration data.
    profiles = np.array(
        [
            [0.0, 1.50, 36.0, 2200.0],
            [0.25, 1.20, 28.0, 1400.0],
            [0.50, 0.90, 18.0, 700.0],
            [0.75, 0.70, 12.0, 400.0],
            [1.00, 0.55, 8.0, 250.0],
        ],
        dtype=np.float64,
    )
    return MotionLimits(
        max_speed=float(np.interp(strength, profiles[:, 0], profiles[:, 1])),
        max_acceleration=float(np.interp(strength, profiles[:, 0], profiles[:, 2])),
        max_jerk=float(np.interp(strength, profiles[:, 0], profiles[:, 3])),
    )


class MocapCommandLimiter:
    """Rate-limit mocap positions by speed, acceleration, and jerk.
    ...
    joint-state estimate. Quaternion commands are intentionally not filtered.
    """

    def __init__(self, config: ClientConfig):
        self.dt = 1.0 / config.send_fps
        limits = limits_from_smoothing_strength(config.motion_smoothing)
        if limits is None:
            raise ValueError("MocapCommandLimiter must not be created when motion_smoothing is 0")
        self.max_linear_speed = limits.max_speed
        self.max_linear_acceleration = limits.max_acceleration
        self.max_linear_jerk = limits.max_jerk
        ...
        logger.info(
            "Mocap position smoothing=%.2f -> speed=%.2f m/s, accel=%.1f m/s^2, jerk=%.0f m/s^3",
    ...
    def step(self, target_pose7: np.ndarray) -> np.ndarray:
        ...
        output[:, :3] = output_xyz
        output[:, 3:] = target_pose7[:, 3:]
        output = smooth_pose7_quat_sign(output[None, ...])[0]
        self.last_pose7 = output.astype(np.float64)
        if any((linear_jerk_limited, linear_acceleration_limited, linear_speed_limited)):
            logger.warning(
                "Mocap command %d position-limited: jerk=%d, accel=%d, speed=%d out of %d points",
                self.command_index,
                linear_jerk_limited,
...
if __name__ == "__main__":
    ...
    limits_from_smoothing_strength(config.motion_smoothing)
    ...
    command_limiter = (
        MocapCommandLimiter(config) if config.motion_smoothing > 0.0 else None
    )
    if command_limiter is None:
        logger.info("Mocap position smoothing is disabled; raw actions will be sent.")
        ...
        for idx in range(config.roll_out):
            ...
            for t in range(action_chunk_abs.shape[0]):
                ...
                mocap_frame = (
                    ...
                    if command_limiter is not None
