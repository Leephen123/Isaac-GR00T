...
class ClientConfig:
    ...
    interp_num: int = 1
    # Smooth the beginning of each installed RTC chunk without changing its
    # frame count or modifying dexterous-hand actions.
...
def blend_chunk_boundary(
    ...
    """Smooth a chunk boundary without inserting or removing frames."""
...
class Gr00tRtcV2Runner:
    ...
    def _install_chunk(
        ...
        # Blend from the last command to the first frame that RTC will actually
        # send after applying any overlap skip or interpolation bridge.
        if self.config.enable_chunk_boundary_blend:
            frames = blend_chunk_boundary(
                prev_last_pose7=self.last_sent_pose,
                cur_chunk_pose7=frames,
                blend_frames=self.config.chunk_boundary_blend_frames,
            )
