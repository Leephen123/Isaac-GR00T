...
from data_res.camera import CameraGrabber, VideoCapture
...
class ClientConfig:
    ...
    camera_frame_age_warn_ms: float = 200.0
...
def build_action_frames(
    ...
    """Build synchronized action frames with separate intra/inter chunk counts.

    The returned ``prepended_bridge_frames`` count lets the RTC bookkeeping exclude
    inter-chunk inserted frames. Intra-chunk inserted frames remain in the action
    timeline, where ``interp_stride`` identifies only the original model frames.
    """
    ...
    if cur_chunk_hand.ndim != 2 or cur_chunk_hand.shape[1] != 12:
        raise ValueError(
            f"Expected cur_chunk_hand shape (T,12), got {cur_chunk_hand.shape}"
        )
    ...
    if prev_last_pose7 is not None and prev_last_hand is not None:
        ...
        if prev_pose.shape != (15, 7):
            raise ValueError(
                f"Expected prev_last_pose7 shape (15,7), got {prev_pose.shape}"
            )
        if prev_hand.shape != (12,):
            raise ValueError(
                f"Expected prev_last_hand shape (12,), got {prev_hand.shape}"
            )

    if not enable_interp:
        return cur_chunk_pose7, cur_chunk_hand, 0

    if intra_chunk_interp_num > 0:
        chunk_frames = interpolate_pose7(
            cur_chunk_pose7,
            num_interp=intra_chunk_interp_num,
        )
        chunk_hand_frames = interpolate_hand_joint(
            cur_chunk_hand,
            num_interp=intra_chunk_interp_num,
        )
    else:
        chunk_frames = cur_chunk_pose7
        chunk_hand_frames = cur_chunk_hand

    prepended_bridge_frames = 0
    frames = chunk_frames
    hand_frames = chunk_hand_frames
    if (
        allow_bridge
        ...
        bridge_frames = interpolate_pose7(
            ...
            num_interp=inter_chunk_interp_num,
        )
        bridge_hand_frames = interpolate_hand_joint(
            np.stack([prev_hand, cur_chunk_hand[0]], axis=0),
            num_interp=inter_chunk_interp_num,
        )
        inserted_bridge_frames = bridge_frames[1:-1]
        inserted_bridge_hands = bridge_hand_frames[1:-1]
        prepended_bridge_frames = inserted_bridge_frames.shape[0]
        frames = np.concatenate([inserted_bridge_frames, chunk_frames], axis=0)
        hand_frames = np.concatenate(
            [inserted_bridge_hands, chunk_hand_frames],
            axis=0,
        )

    logger.info(
        "action interpolation enabled: bridge=%d, intra_per_gap=%d, "
        "model_steps=%d, frames=%d",
...
def validate_config(config: ClientConfig) -> None:
    ...
    if not np.isfinite(config.camera_ready_timeout_s) or (
        config.camera_ready_timeout_s <= 0
    ):
        raise ValueError(
            "camera_ready_timeout_s must be positive, "
            f"got {config.camera_ready_timeout_s}"
        )
    if not np.isfinite(config.camera_frame_age_warn_ms) or (
        config.camera_frame_age_warn_ms <= 0
    ):
        raise ValueError(
            "camera_frame_age_warn_ms must be positive, "
            f"got {config.camera_frame_age_warn_ms}"
        )
...
class Gr00tRtcV2Runner:
    def __init__(self, config: ClientConfig):
        self.config = config
        self.interp_stride = (
            config.intra_chunk_interp_num + 1
            if config.enable_action_interp
            else 1
        )
    ...
    def _initialize(self) -> None:
        for name in get_camera_name(self.config.camera_config):
            self.camera_caps[name] = VideoCapture(name)
        self.camera_grabber = CameraGrabber(self.camera_caps)
        self.camera_grabber.start()
        self.camera_grabber.wait_until_ready(
            timeout_s=self.config.camera_ready_timeout_s
        )
        logger.info(
            "background camera capture ready: cameras=%s",
            sorted(self.camera_caps),
        )
    ...
    def _infer_once(
        ...
        snapshot_start_s = monotonic()
        frame, captured_at_s = self.camera_grabber.get_snapshot()
        snapshot_at_s = monotonic()
        snapshot_copy_ms = (snapshot_at_s - snapshot_start_s) * 1000.0
        frame_age_ms = {
            name: max(0.0, snapshot_at_s - timestamp) * 1000.0
            for name, timestamp in captured_at_s.items()
        }
        max_frame_age_ms = max(frame_age_ms.values(), default=0.0)
        capture_skew_ms = (
            (max(captured_at_s.values()) - min(captured_at_s.values())) * 1000.0
            if len(captured_at_s) > 1
            else 0.0
        )
        log_camera_snapshot = (
            logger.warning
            if max_frame_age_ms > self.config.camera_frame_age_warn_ms
            else logger.info
        )
        log_camera_snapshot(
            "camera snapshot: copy_ms=%.2f, max_age_ms=%.2f, "
            "capture_skew_ms=%.2f, ages_ms=%s",
            snapshot_copy_ms,
            max_frame_age_ms,
            capture_skew_ms,
            {name: round(age_ms, 2) for name, age_ms in frame_age_ms.items()},
        )
    ...
    def _install_chunk(
        ...
        full_frames, full_hands, prepended_bridge_frames = build_action_frames(
            ...
            intra_chunk_interp_num=self.config.intra_chunk_interp_num,
            inter_chunk_interp_num=self.config.inter_chunk_interp_num,
        )
    ...
    def _maybe_submit_rtc(self) -> None:
        ...
        if ok:
            ...
            logger.info(
                ...
                "remaining_steps=%d, overlap=%d, frozen=%d",
                self.active.chunk_id,
                action_exec_s,
                remaining_steps,
                overlap_steps,
    ...
    def _sample_history(self) -> None:
        # Bridge frames do not advance full_frame_count. Within a chunk, only
        # original model frames land on an interp_stride boundary, so neither
        # kind of inserted frame is written to history.
