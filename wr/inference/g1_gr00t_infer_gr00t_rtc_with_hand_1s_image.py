...
class ImageHistoryQueue:
    ...
    def put(self, frame: dict[str, np.ndarray]) -> None:
        with self._lock:
    ...
    def get(self) -> list[dict[str, np.ndarray]]:
        with self._lock:
            if not self._queue:
                raise RuntimeError("Image history queue is empty")
...
def build_observation_from_msg_with_history(
    ...
    task_description: str,
    frame_history: list[dict[str, np.ndarray]] | None,
) -> dict:
    ...
    if frame_history is not None:
        for name in frame_history[0]:
            video[CAMERAS_MAP[name]] = np.stack(
                [
                    load_image_from_array(np.asarray(frame[name]), (256, 256))[0, 0]
...
def validate_config(config: ClientConfig) -> None:
    ...
    if config.model_step_fps <= 0:
        raise ValueError(
            f"model_step_fps must be positive, got {config.model_step_fps}"
        )
...
class Gr00tRtcV2Runner:
    def __init__(self, config: ClientConfig):
        ...
        self.image_history = ImageHistoryQueue(config.image_history_indices)
    ...
    def _initialize(self) -> None:
        ...
        self._sample_image_history(force=True)
        while True:
    ...
    def _infer_once(
        ...
        frame_history = self.image_history.get()
    ...
    def _sample_image_history(self, force: bool = False) -> None:
        if not force:
            self._image_sample_acc += (
                self.config.camera_fps / self.config.model_step_fps
            )
