...
class G1Env():
    def __init__(self, sim2, config, hands: str):
        ...
        if self.config.robot in ["g1", "k2", "sber", "pm"]:
            ...
            raise ValueError(f"Invalid robot: {self.config.robot}")
        ...
        if self.config.robot in ["g1", "k2", "sber", "pm"]:
            ...
            assert self.hands in ["box", "dex3"], "Only support dex3 and box hands now"
            if self.hands == "dex3":
    ...
    def visualize(self, visualization):
        if self.sim2 == "real":
            return
        if (not visualization) or (visualization.get("keypoints", None) is None):
            return
